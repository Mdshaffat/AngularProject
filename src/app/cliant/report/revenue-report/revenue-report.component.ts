...
import { AllRevenue, Revenue } from 'src/app/core/models/Report/revenue';
...
export class RevenueReportComponent implements OnInit, AfterViewInit {
  ...
  footerName = 'Report';
  newPatientsCreatedByDateAndBranch: AllRevenue[] = [];
  revenueReportWithTotals: Revenue;
  patientSearchFormByDate: FormGroup = new FormGroup({});
  displayedColumns: string[] = ['BranchName', 'Doctor', 'Member', 'NonMember',
                                 'Revenue'];
  dataSource = new MatTableDataSource<AllRevenue>(this.newPatientsCreatedByDateAndBranch);
  ...
  patientCountAccordingToDateAndBranch(){
    ...
    if (this.patientSearchFormByDate.value.startDate && this.patientSearchFormByDate.value.endDate){
      ...
      this.reportService.getRevenueAccordingToDateAndBranch(startDate, endDate).subscribe(response => {
        this.revenueReportWithTotals = response;
        this.newPatientsCreatedByDateAndBranch = response.allRevenue;
        this.dataSource = new MatTableDataSource<AllRevenue>(this.newPatientsCreatedByDateAndBranch);
        response.allRevenue.forEach((res) => {
          this.doctorsName.push(res.doctorsName + '(' + res.branchName + ')');
          this.malePatient.push(res.member);
          this.femalePatient.push(res.nonMember);
          this.totalPatient.push(res.totalAmount);
        });
      ...
      this.dataSource = new MatTableDataSource<AllRevenue>();
    }
  }
  exportArray() {
