...
@Component({
  selector: 'app-todays-list',
  templateUrl: './todays-list.component.html',
  styleUrls: ['./todays-list.component.css']
})
export class TodaysListComponent implements OnInit, AfterViewInit {
  ...
  constructor(private pregnancyService: PregnancyService,
              ...
              public dialog: MatDialog) {
                ...
                .pipe(
                  ...
                  switchMap(value => this.pregnancyService.getAllTodayPregnancy(value, 'nameAsc', 1, 50, this.hospitalId)
                  )).subscribe(response => {
  ...
  motherListAccordingToHospital(hospitalId) {
    if (this.filterValue){
      this.pregnancyService.getAllTodayPregnancy(this.filterValue, '', this.currentPage, this.pageSize, hospitalId).subscribe(response => {
        this.followUps = response.data;
      ...
      this.pregnancyService.getAllTodayPregnancy('', '', this.currentPage, this.pageSize, hospitalId).subscribe(response => {
        this.followUps = response.data;
        this.totalRows = response.count;
        this.dataSource.data = response.data;
        setTimeout(() => {
  ...
  applyFilter(event: Event) {
    ...
    setTimeout(() => {
      this.pregnancyService.getAllTodayPregnancy(this.filterValue, 'nameAsc', 1, 50, this.hospitalId).subscribe(response => {
       this.followUps = response.data;
       this.dataSource.data = response.data;
       setTimeout(() => {
         this.paginator.length = response.count;
